...
const migrations = migrationNames.split(',').map(name => {
  ...
  if (!migration) {
    throw new Error(`Unknown migration name "${name}", available migrations: ${Object.keys(ALL_MIGRATIONS)}`);
  }
  return migration;
});

if (!migrations.length) {
  throw new Error(`No migrations found, available migrations: ${Object.keys(ALL_MIGRATIONS)}`);
}

const getPage = skip => {
  console.log(`Getting 100 docs from ${skip}`);
  return db.allDocs({
    include_docs: true,
    limit: PAGE_LIMIT,
    skip: skip
...
const updateDocs = docs => {
  console.log(`Executing ${migrations.length} migration(s) on ${docs.length} doc(s)`);
  return docs.filter(doc => migrations.some(migration => migration(doc)));
};

const saveUpdated = docs => {
  if (!docs.length) {
    return console.log(`No docs in page required migration`);
  }
  console.log(`Updating ${docs.length} doc(s)`);
...
const run = skip => {
  getPage(skip)
    .then(results => {
      ...
      if (!docs.length) {
        console.log('No more docs found - finished successfully');
        process.exit();
      }
      return docs;
    })
