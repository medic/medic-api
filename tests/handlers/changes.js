  ...
  var testRes = {
    ...
    write: function(slice) {
      ...
      setTimeout(function() { // timeout to make sure nothing else tries to respond
        ...
        test.deepEqual(db.request.args[0][0].body.doc_ids.sort(), [ deletedId, unchangedId, allowedId, userId ].sort());
  ...
  var testRes2 = {
    ...
    write: function(slice) {
      ...
      setTimeout(function() { // timeout to make sure nothing else tries to respond
        ...
        test.deepEqual(db.request.args[0][0].body.doc_ids.sort(), [ unchangedId, userId1 ].sort());
        test.deepEqual(db.request.args[1][0].body.doc_ids.sort(), [ unchangedId, userId2 ].sort());
        test.deepEqual(db.request.args[2][0].body.doc_ids.sort(), [ unchangedId, userId2, newId ].sort());
