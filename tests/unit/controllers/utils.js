...
var dateTests = [
  ...
  {
    // accepts 2 digit timezone
    val: '2011-10-10T14:48:00-03',
    res: [true, 1318268880000]
  },
  {
    // accept 4 digit timezone
    val: '2011-10-10T14:48:00-0330',
    res: [true, 1318270680000]
  },
  {
    // accept Z for UTC
    val: '2011-10-10T14:48:00Z',
    res: [true, 1318258080000]
  },
  {
    // support accept ms since epoch
    val: '0',
    res: [true, 0]
  },
  {
  ...
  {
    // accept ms since epoch (string)
    val: '1467383343484',
    res: [true, 1467383343484]
  },
  {
    // accept ms since epoch
    val: 1467383343484,
  ...
  },
  {
    ...
    val: new Date(now).valueOf(),
    res: [true, now.valueOf()]
  },
  {
    // accept output from native javascript method
    val: '2016-07-01T14:58:26.336Z',
    res: [true, 1467385106336]
  },
  {
    // accept output from native javascript method
    val: new Date(now).toISOString(),
    res: [true, now.valueOf()]
  }
];
...
exports['describe isDateStrValid'] = function(test) {
  test.expect(dateTests.length);
  dateTests.forEach(function(t) {
    test.equals(t.res[0], utils.isDateStrValid(t.val));
  });
  test.done();
};

exports['describe parseDate'] = function(test) {
  test.expect(dateTests.length);
  dateTests.forEach(function(t) {
    if (t.res[1] === 'falsey') {
      test.equals(false, Boolean(utils.parseDate(t.val).valueOf()));
    } else {
      test.equals(t.res[1], utils.parseDate(t.val).valueOf());
    }
  });
  test.done();
};
